...
class CSVAnalyzer {
    ...
    init() {
        ...
        document.getElementById('selectAllColumns').addEventListener('click', () => this.selectAllColumns(true));
        document.getElementById('deselectAllColumns').addEventListener('click', () => this.selectAllColumns(false));

        // Preset buttons (alleen voor actieve presets)
        document.getElementById('presetBasic').addEventListener('click', () => this.applyPreset('basic'));
        ...
        document.getElementById('clearPreset').addEventListener('click', () => this.clearCustomPreset());
        ...
        // Column checkbox change listeners (voor real-time preview update)
        document.addEventListener('change', (e) => {
            if (e.target.matches('#columnCheckboxes input[type="checkbox"]')) {
                this.updatePreview();
            }
        });
    ...
    updateColumnSelection() {
        ...
        const columnSection = document.querySelector('.column-selection');
        if (!columnSection) return;
        
        // Zorg ervoor dat de column selection zichtbaar is
        columnSection.style.display = 'block';

        // Toon ook de tracking section
        ...
        this.baseData.headers.forEach(header => {
            const checkboxDiv = document.createElement('div');
            checkboxDiv.className = 'column-checkbox';
            
            // Special styling voor de Nummer kolom (klantnummer)
            if (header === 'Nummer') {
                checkboxDiv.style.backgroundColor = '#e8f4fd';
                checkboxDiv.style.border = '2px solid #667eea';
            ...
            // Bepaal of deze kolom geselecteerd moet zijn
            ...
            // Bewaar huidige selectie als deze update wordt aangeroepen na initiële load
            const existingCheckbox = document.getElementById(`col_${header}`);
            const wasChecked = existingCheckbox ? existingCheckbox.checked : false;
            
            // Voor nieuwe headers: trackingColumns en Nummer standaard aan, anders behoud selectie
            if (existingCheckbox) {
                // Update scenario: behoud bestaande selectie
                checkbox.checked = wasChecked;
            } else {
                // Nieuwe kolom scenario: alleen Nummer en tracking kolommen standaard aan
                checkbox.checked = (header === 'Nummer' || trackingColumns.includes(header));
            }
            
            // Maak de Nummer kolom (klantnummer) verplicht
            if (header === 'Nummer') {
                checkbox.checked = true;
                checkbox.disabled = true; // Kan niet worden uitgevinkt
            }
            
            // Opmerking: Event listener wordt al afgehandeld door de delegated listener in init()
            
            const label = document.createElement('label');
            label.htmlFor = `col_${header}`;
            label.textContent = header;
            
            // Voeg een indicatie toe voor de verplichte kolom
            if (header === 'Nummer') {
                const required = document.createElement('span');
                required.textContent = ' (Klantnummer - Verplicht)';
                required.style.color = '#667eea';
                required.style.fontSize = '0.8rem';
        ...
        setTimeout(() => {
            this.updatePreview();
        }, 100);        // Check custom preset beschikbaarheid
        this.checkCustomPresetAvailable();
    }

    getSelectedColumns() {
        // Haal geselecteerde gewone kolommen op
        ...
        // Filter out tracking kolommen uit de gewone selectie (ze zitten niet in columnCheckboxes)
        const trackingColumnNames = ['Inventarisatie OWR gemaakt', 'OWR zinvol', 'Gegevens compleet', 'Contact met klant opgenomen', 'Opgave OWR ingevuld', 'Afgewerkt'];
        selectedColumns = selectedColumns.filter(col => !trackingColumnNames.includes(col));
        
        // Zorg ervoor dat "Nummer" (klantnummer) altijd als eerste kolom verschijnt indien geselecteerd
        if (this.baseData && this.baseData.headers.includes('Nummer') && selectedColumns.includes('Nummer')) {
            // Verwijder "Nummer" uit de lijst (als het er al in staat)
            selectedColumns = selectedColumns.filter(col => col !== 'Nummer');
            // Voeg "Nummer" toe als eerste kolom
            selectedColumns.unshift('Nummer');
        }
